...
class SessionManager:
    """Manages Telegram user session for monitoring groups and processing cards"""
    
    def __init__(self, db: NetflixDatabase):
        self.db = db
        self.client = None
    ...
    async def load_session(self, session_string: str) -> bool:
        ...
        try:
            ...
            if len(session_string) < 10:
                logger.error("Invalid session string format - too short")
                return False
            ...
            logger.error(f"API ID conversion error: {e}")
            return False
    ...
    async def _process_message(self, event, chat_id: str):
        ...
        logger.debug(f"Processing message from group {chat_id}: {message_text[:20]}...")
        
        # Check for card information in the message
        card_info = self.card_detector.detect_credit_card_info(message_text)
        
        if card_info:
            logger.info(f"Credit card detected in group {chat_id}: {card_info['card_number'][:6]}****")
            ...
            try:
                ...
                logger.info(f"Added card to database with ID: {card_id}")
                ...
                if not self.processing_lock.locked():
                    logger.info("Starting account processing task")
                    asyncio.create_task(self.process_accounts())
                else:
                    logger.info("Account processing already in progress")
            except Exception as e:
                logger.error(f"Error adding card to database: {e}", exc_info=True)
    ...
    async def _process_single_account(self, account: Dict[str, Any]):
        ...
        try:
            ...
            try:
                ...
                # First try - attempt with cookies if available
                if account['cookies']:
                    # Try to use cookies if available
                    cookie_success = netflix.load_cookies(account['cookies'])
                    logger.info(f"Loaded cookies for account {account['email']}: {'Success' if cookie_success else 'Failed'}")
                
                # If no cookies or cookie login failed, try with credentials
                current_page = netflix.detect_current_page()
                if current_page == 'login' or current_page == 'unknown':
                    # Login with credentials
                    login_result = await netflix.login_with_credentials(account['email'], account['password'])
...
async def handle_callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if callback_data == MENU_CALLBACK["MAIN_MENU"]:
        ...
        await handle_setting_option(query, context, callback_data.split(":")[1])
...
async def handle_setting_option(query, context, option):
    """Handle various settings options"""
    session_manager = context.bot_data.get("session_manager")
    db = context.bot_data.get("db")
    
    if option == "start_monitoring":
        if not session_manager:
            await query.edit_message_text(
                "Session manager not initialized. Please try again later.",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("Back to Settings", callback_data=MENU_CALLBACK["SETTINGS"])
                ]])
            )
            return
            
        success = await session_manager.start_monitoring()
        if success:
            await query.edit_message_text(
                "✅ Monitoring started successfully! The bot is now watching configured groups for cards.",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("Back to Settings", callback_data=MENU_CALLBACK["SETTINGS"])
                ]])
            )
            ...
            await query.edit_message_text(
                "❌ Failed to start monitoring. Please make sure you've loaded a valid session.",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("Load Session", callback_data=MENU_CALLBACK["LOAD_SESSION"]),
                    InlineKeyboardButton("Back to Settings", callback_data=MENU_CALLBACK["SETTINGS"])
                ]])
        ...
        await query.edit_message_text(
            "✅ Monitoring stopped. The bot is no longer watching groups for cards.",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("Back to Settings", callback_data=MENU_CALLBACK["SETTINGS"])
            ]])
        )
    
    elif option == "remove_session":
        if not db:
            await query.edit_message_text(
                "Database not initialized. Please try again later.",
                reply_markup=InlineKeyboardMarkup([[
        ...
        if session_string:
            ...
            db.deactivate_session(1)  # Assuming the active session has ID 1
            ...
            await query.edit_message_text(
                "✅ Session removed successfully. You'll need to load a new session to continue.",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("Load Session", callback_data=MENU_CALLBACK["LOAD_SESSION"]),
                    InlineKeyboardButton("Back to Settings", callback_data=MENU_CALLBACK["SETTINGS"])
                ]])
...
async def main() -> None:
    ...
    if not telegram_api_id or not telegram_api_hash:
        logger.warning("Telegram API credentials not set. Session functionality will be limited.")
